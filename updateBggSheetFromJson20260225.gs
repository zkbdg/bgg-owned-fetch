...
function updateBggSheetFromJson() {
  ...
  if (finalRows.length > 0) {
    ...
    const bgColors = finalRows.map(row => {
      const status = row[17];
      let color = "#ffffff";
      if (status === "preordered") color = "#e6ffed";        // 薄い緑
      else if (status === "wishlist") color = "#fff3e0";      // 薄いオレンジ
      else if (status === "previouslyowned") color = "#f5f5f5"; // 薄いグレー
      else if (status === "played(not owned)") color = "#e3f2fd"; // 薄い青 ★追加箇所
