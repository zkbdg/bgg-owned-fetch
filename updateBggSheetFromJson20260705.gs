...
function updateBggSheetFromJson() {
  ...
  let finalRows = data.map(item => {
    ...
    // JSON直下、あるいは stats 内のどちらにあっても対応できるように抽出
    ...
    return [
      ...
      stats.minplayers, stats.maxplayers, stats.playingtime, 
      minAgeVal, // ★「プレイ時間」と「出版年」の間に対象年齢データを挿入
      extractValue(item.yearpublished),
  ...
  finalRows.sort((a, b) => {
    ...
    // ★ステータスの位置が1つずれたため 17 ➔ 18 に変更
    const orderA = statusOrder[a[18]] || 99, orderB = statusOrder[b[18]] || 99;
    if (orderA !== orderB) return orderA - orderB;
    return a[0].localeCompare(b[0]);
  });

  // ★「対象年齢」ヘッダーを追加
  const headers = ["ゲーム名", "プレイ回数", "最終プレイ日", "マイ評価", "平均評価", "ベイズ平均", "Board Game Rank", "Family Game Rank", "Weight", "最小人数", "最大人数", "プレイ時間", "対象年齢", "出版年", "デザイナー", "メカニクス", "カテゴリー", "タイプ", "ステータス", "ID"];
  ...
  if (finalRows.length > 0) {
    ...
    // ★IDの位置が1つずれたため 18 ➔ 19 に変更
    const richTexts = finalRows.map(row => [
      SpreadsheetApp.newRichTextValue().setText(row[0]).setLinkUrl(`https://boardgamegeek.com/boardgame/${row[19]}`).build()
    ]);
    ...
    // ★対象年齢が入ったため、数値フォーマット（整数）の適用列数を 4 ➔ 5 に変更
    sheet.getRange(3, 10, totalRows, 5).setNumberFormat("0");
  ...
  // ★列幅配列に「対象年齢」の 60 を追加
  const widths = [250, 60, 100, 60, 70, 100, 120, 120, 80, 80, 80, 90, 60, 60, 150, 250, 200, 100, 100, 60];
